...
void cleanup_theme_binding(lv_event_t* event) {
  auto* obj = lv_event_get_target_obj(event);
  if (obj) {
    lv_anim_delete(obj, nullptr);
  }
  delete static_cast<ThemeBinding*>(lv_event_get_user_data(event));
}
...
void theme_observer(lv_observer_t* observer, lv_subject_t* subject) {
  ...
  auto* binding = static_cast<ThemeBinding*>(lv_observer_get_user_data(observer));
  if (!binding) {
    return;
  }
  const auto role    = binding->role;
  const auto colors  = view::palette(lv_subject_get_int(subject) != 0);
  const bool animate = binding->initialized;
  ...
  switch (role) {
    case ThemeRole::SCREEN:
      ...
      if (animate) {
        view::animate_style_color(obj, LV_STYLE_BG_COLOR, colors.background);
        view::animate_style_color(obj, LV_STYLE_TEXT_COLOR, colors.text);
      } else {
    ...
    case ThemeRole::SURFACE:
      ...
      lv_obj_set_style_bg_opa(obj, LV_OPA_COVER, 0);
      if (animate) {
        view::animate_style_color(obj, LV_STYLE_BG_COLOR, colors.surface);
        view::animate_style_color(obj, LV_STYLE_TEXT_COLOR, colors.text);
      } else {
    ...
    case ThemeRole::BAR:
      ...
      if (animate) {
        view::animate_style_color(obj, LV_STYLE_BG_COLOR, colors.bar);
        view::animate_style_color(obj, LV_STYLE_BORDER_COLOR, colors.border);
        view::animate_style_color(obj, LV_STYLE_TEXT_COLOR, colors.text);
      } else {
    ...
    case ThemeRole::BUTTON:
      ...
      lv_obj_set_style_outline_color(obj, colors.border, 0);
      lv_obj_set_style_bg_opa(obj, LV_OPA_COVER, 0);
      lv_obj_set_style_pad_hor(obj, 8, 0);
      lv_obj_set_style_pad_ver(obj, 3, 0);
      if (animate) {
        view::animate_style_color(obj, LV_STYLE_BORDER_COLOR, colors.border);
        view::animate_style_color(obj, LV_STYLE_BG_COLOR, colors.button);
        view::animate_style_color(obj, LV_STYLE_TEXT_COLOR, colors.text);
      } else {
        lv_obj_set_style_border_color(obj, colors.border, 0);
...
void bind_theme(lv_obj_t* obj, lv_subject_t* dark_mode_subject, ThemeRole role) {
  ...
  auto* binding = new ThemeBinding{role, false};
  lv_obj_add_event_cb(obj, cleanup_theme_binding, LV_EVENT_DELETE, binding);
  lv_subject_add_observer_obj(dark_mode_subject, theme_observer, obj, binding);
}
...
void on_event_set_string(lv_obj_t* obj,
                         lv_subject_t* subject,
