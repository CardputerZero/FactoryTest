...
class StringSubject final : public Subject {
 public:
  ...
  const char* value() const { return lv_subject_get_string(native_mutable_()); }

  const char* previous() const { return lv_subject_get_previous_string(native_mutable_()); }

  void set(const char* value) { lv_subject_copy_string(native(), value ? value : ""); }

 protected:
  lv_subject_t* native_mutable_() const { return const_cast<lv_subject_t*>(&subject_); }
...
class PointerSubject final : public Subject {
 public:
  ...
  T* value() const {
    return const_cast<T*>(reinterpret_cast<const T*>(lv_subject_get_pointer(native_mutable_())));
  }

  T* previous() const {
    return const_cast<T*>(
        reinterpret_cast<const T*>(lv_subject_get_previous_pointer(native_mutable_())));
  }

  void set(T* value) { lv_subject_set_pointer(native(), value); }
