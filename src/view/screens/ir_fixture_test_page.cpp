...
lv_color_t state_color(model::IrFixtureItemState state, bool dark_mode) {
  const auto colors = view::palette(dark_mode);
  switch (state) {
    case model::IrFixtureItemState::RUNNING:
      return colors.info;
    case model::IrFixtureItemState::PASSED:
      return colors.success;
    case model::IrFixtureItemState::FAILED:
      return colors.error;
    case model::IrFixtureItemState::PENDING:
    default:
      return colors.text_muted;
  }
}

bool is_enter(uint32_t key, const char* key_name) {
  return key == LV_KEY_ENTER || key == '\n' || key == '\r' ||
         (key_name &&
          (std::strcmp(key_name, "Enter") == 0 || std::strcmp(key_name, "Return") == 0));
}

bool can_start(model::IrFixtureRunState state) {
  return state != model::IrFixtureRunState::RUNNING;
}

}  // namespace

IrFixtureTestPage::IrFixtureTestPage(viewmodel::AppViewModel& app_view_model,
...
IrFixtureTestPage::~IrFixtureTestPage() {
...
void IrFixtureTestPage::build_content(lv_obj_t* content) {
  auto* title_font =
  ...
  const auto initial_headline = app_view_model_ref_().tr("Prepare IR fixture, then press Enter");
  ...
  for (std::size_t i = 0; i < initial.items.size(); ++i) {
    ...
    lv_obj_set_width(detail_values_[i], 194);
...
void IrFixtureTestPage::refresh_timer_cb(lv_timer_t* timer) {
  auto* page = static_cast<IrFixtureTestPage*>(lv_timer_get_user_data(timer));
  if (page) {
    page->refresh_();
  }
}

void IrFixtureTestPage::key_listener(uint32_t key, const char* key_name, void* user_data) {
  auto* page = static_cast<IrFixtureTestPage*>(user_data);
  if (!page) {
    return;
  }
...
void IrFixtureTestPage::refresh_() {
  ...
  for (std::size_t i = 0; i < snapshot.items.size(); ++i) {
    const auto translated_state =
        app_view_model_ref_().tr(model::ir_fixture_item_state_text(snapshot.items[i].state));
