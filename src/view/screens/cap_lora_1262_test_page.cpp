...
lv_color_t state_color(model::CapLora1262ItemState state, bool dark_mode) {
  const auto colors = view::palette(dark_mode);
  switch (state) {
    case model::CapLora1262ItemState::RUNNING:
      return colors.info;
    case model::CapLora1262ItemState::PASSED:
      return colors.success;
    case model::CapLora1262ItemState::FAILED:
      return colors.error;
    case model::CapLora1262ItemState::PENDING:
    default:
      return colors.text_muted;
  }
}

bool can_start(model::CapLora1262RunState state) {
  return state == model::CapLora1262RunState::WAITING_FOR_CAP ||
         state == model::CapLora1262RunState::PASSED ||
         state == model::CapLora1262RunState::FAILED ||
         state == model::CapLora1262RunState::CANCELED;
}

}  // namespace

CapLora1262TestPage::CapLora1262TestPage(viewmodel::AppViewModel& app_view_model,
...
CapLora1262TestPage::~CapLora1262TestPage() {
...
void CapLora1262TestPage::build_content(lv_obj_t* content) {
  auto* title_font =
  ...
  lv_obj_set_style_pad_row(layout, 3, 0);
  lv_obj_clear_flag(layout, LV_OBJ_FLAG_SCROLLABLE);

  status_label_               = lv_label_create(layout);
  const auto initial_headline = app_view_model_ref_().tr("Install CAP LoRa-1262, then press Enter");
...
void CapLora1262TestPage::refresh_timer_cb(lv_timer_t* timer) {
  auto* page = static_cast<CapLora1262TestPage*>(lv_timer_get_user_data(timer));
  if (page) {
    page->refresh_();
  }
}

void CapLora1262TestPage::key_listener(uint32_t key, const char* key_name, void* user_data) {
  auto* page = static_cast<CapLora1262TestPage*>(user_data);
  if (!page) {
    return;
  }
...
void CapLora1262TestPage::refresh_() {
  ...
  for (std::size_t i = 0; i < snapshot.items.size(); ++i) {
    const auto translated_state =
        app_view_model_ref_().tr(cap_lora_1262_item_state_text(snapshot.items[i].state));
...
void CapLora1262TestPage::cancel_and_leave_() {
  model_.cancel();
