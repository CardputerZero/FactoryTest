...
void Logger::set_tag(const char* tag) { current_tag = tag ? tag : ""; }

const char* Logger::tag() { return current_tag; }

void Logger::set_color_mode(ColorMode mode) { current_color_mode = mode; }

void Logger::set_timestamp_enabled(bool enabled) { timestamp_enabled_flag = enabled; }

bool Logger::timestamp_enabled() { return timestamp_enabled_flag; }

// conditional log
bool Logger::should_log(LogLevel level) {
  return level >= current_level && current_level != LogLevel::OFF;
}
...
void Logger::log(LogLevel level, const std::string& msg, const char* file, int line) {
  ...
  std::string prefix = make_prefix(level, current_tag, timestamp_enabled_flag, file, line);
