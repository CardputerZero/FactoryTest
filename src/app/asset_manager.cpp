...
#ifndef APP_NAME
#define APP_NAME "factory_test_app"
#endif
...
AssetManager::AssetManager(std::string_view app_name, std::filesystem::path cmake_assets_root) {
  add_root_if_valid_(std::move(cmake_assets_root));
  add_root_if_valid_(default_cmake_assets_root());
  add_root_if_valid_(APP_SOURCE_ASSETS_ROOT);
  add_root_if_valid_(std::filesystem::path{"/usr/share"} / std::string(app_name));
}
...
std::string AssetManager::default_app_name() { return APP_NAME; }

std::filesystem::path AssetManager::default_cmake_assets_root() { return APP_CMAKE_ASSETS_ROOT; }

const std::vector<std::filesystem::path>& AssetManager::roots() const { return roots_; }
...
lv_font_t* AssetManager::load_font(const std::filesystem::path& file_name, uint32_t size) {
  ...
  auto existing =
      std::find_if(loaded_fonts_.begin(), loaded_fonts_.end(), [&](const auto& loaded_font) {
...
void AssetManager::add_root_if_valid_(std::filesystem::path path) {
  if (path.empty()) {
    return;
  }
